...
function validateCardNum() {
	...
	if (!checkCardNum(numbers)) {
		setError("Credit Card Number is incorrect. Please, re-type it again.");
		return false;
	} else
		return true;
}
...
function checkIssueNum() {
	...
	if (!input.match(pattern)) {
		setError("Issue Number for Maestro card is incorrect. Please, re-type it again.");
		return false;
	} else
		return true;
}
...
function checkCvv2() {
	...
	if (!(input.match(pattern1) || input.match(pattern2))) {
		setError("CVV2 Code is incorrect. Please, re-type it again.");
		return false;
	} else
		return true;
}
...
function checkExpDate() {
	...
	var daysInInputMonth = 32 - new Date(inputYear, (inputMonth - 1), 32).getDate();
	if ((inputYear < currentYear)) {
		console.log(inputYear);
		console.log("bad year");
		setError("Expire Date is incorrect. Please, pick another one.");
		return false;
		...
		if ((daysInInputMonth - currentDay) < 3) {
			setError("Unfortunately you are unable to pay by this Credit Card, since Expire Date is too close.");
			return false;
		}
	} 
		console.log("year return true");
	return true;
}
