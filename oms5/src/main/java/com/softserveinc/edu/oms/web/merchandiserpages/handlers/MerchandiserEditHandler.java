...
public class MerchandiserEditHandler extends AbstractCrudHandler {
	 ...
	 */
	private Date getDeliveryDateFromRequest(final HttpServletRequest request) throws ParseException {
		String dateStr = request.getParameter(MerchandiserPageParameters.DELIVERY_DATE);
	...
	private Boolean getIsGiftFromRequest(final HttpServletRequest request)
			throws Exception {
		String idGiftStr = request.getParameter(MerchandiserPageParameters.IS_GIFT);
		if (idGiftStr == null) {
			return false;
		}
