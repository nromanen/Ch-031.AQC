...
public class OrderDao extends HibernatePageableDao<Order> implements IOrderDao {
	...
	private Criteria addFilter(final Criteria criteria,
		...
		if (role.getId().equals(roleDao.getCustomerRole().getId())) {
		    criteria.add(Restrictions.eq(CUSTOMER_ALIAS + ".login", userName));
		} else if (role.getId().equals(roleDao.getMerchandiserRole().getId())) {
		    criteria.add(Restrictions.eq(ASSIGNEE_ALIAS + ".login", userName));
		}
