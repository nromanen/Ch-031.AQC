...
/**
 * This class used in MerchandiserShowPageController class for realization of
...
public final class MerchandiserPageableHandler extends AbstractPageableHandler {
	 ...
	 */
	@Override
	protected final boolean isValidRequiredParameters(final HttpServletRequest request) throws Exception {
		Boolean result = ParametersValidator.isValidIntegerParameter(request, ParameterNames.ORDER_ID);
		return result;
	}
	...
	protected boolean checkCurrentUserPermissions(
		...
		if (order.getAssigne().equals(currentMerchandiser)) {
			...
			throw new Exception("You can not see order details with another assignee.");
		}
	}
	 ...
	 */
	@Override
	protected final int getNumberOfAllElementsForPageModel(HttpServletRequest request) {
		Order order = getSelectedOrder(request);
	 ...
	 */
	@Override
	protected final void setGeneralPageData(HttpServletRequest request, ModelMap modelMap, PageModel pageModel) {
	 ...
	 */
	@Override
	protected final void setCurrentPageData(HttpServletRequest request, ModelMap modelMap, PageModel pageModel) {
		// Add pageModel with required parameters
		pageModel.getListOfRequiredParameters().add(new ParameterPair(ParameterNames.ORDER_ID, request.getParameter(ParameterNames.ORDER_ID)));
