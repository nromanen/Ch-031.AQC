...
public class UserDao extends HibernatePageableDao<User> implements IUserDao {
	...
	public List<User> findUsersBySearchValue(final String searchValue,
		...
		case REGION:
			Criterion regionCriterion = createRegionCriterion(searchValue,
				condition);
			if (regionCriterion == null) {
				return new LinkedList<User>();
			}
			return findByCriterions(startingFrom, maxResult, sortProperties,
					regionCriterion, IF_IS_ACTIVE);
	...
	public Long countUsersBySearchValue(final String searchValue,
		...
		case REGION:
			...
			return getRowCountByCriterions(regionCriterion, IF_IS_ACTIVE);
	...
	private Criterion[] createFindByValueCriterions(final String value,
		...
		Criterion regionCriterion = createRegionCriterion(value, condition);
		Criterion firstName = condition.createStringCriterion("firstName",value);
		...
		return new Criterion[] { roleCriterion, regionCriterion, firstName,
				lastName, login };
	}

	private Criterion createRegionCriterion(final String value,
		final UserSelectWayCondition condition){
		RegionDao regionDao = new RegionDao();
		regionDao.setSessionFactory(sessionFactory);

		List<Region> regions = regionDao.findRegionByName(value, condition);
		...
		for (Region region : regions) {
			disjunction.add(Restrictions.eq("region", region));
		}
